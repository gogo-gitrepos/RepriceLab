...
import { Store, Plus, CheckCircle, AlertCircle, Loader2, ExternalLink } from 'lucide-react';
...
export default function StoresPage() {
  ...
  const loadStores = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/stores?user_id=2`);
  ...
  const connectAmazon = async () => {
    try {
      ...
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
      const response = await fetch(`${apiUrl}/api/auth/amazon/connect?user_id=2`);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Connect your Amazon and Walmart seller accounts
          </p>
        </div>
      </div>
